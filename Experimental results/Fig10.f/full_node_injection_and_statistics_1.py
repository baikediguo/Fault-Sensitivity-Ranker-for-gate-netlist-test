# full_node_injection_and_statistics.py
# 两阶段故障注入与统计脚本
...
RANK_DIR    = 'gnn_ranks_100runs'  # 存放多个 gnn_rank_xxx_runN.txt 的目录
...
def extract_topk(filename: str) -> int:
    """从文件名提取 TopK 数字，例如 gnn_rank_50.txt -> 50 或 gnn_rank_50_run1.txt -> 50"""
    # 支持两种格式：gnn_rank_50.txt 和 gnn_rank_50_run1.txt
    m = re.search(r'gnn_rank_(\d+)(?:_run\d+)?\.txt', filename)
    if m:
        return int(m.group(1))
    # 也支持 gnn_rank_all_run1.txt 格式
    m2 = re.search(r'gnn_rank_all(?:_run\d+)?\.txt', filename)
    if m2:
...
def phase2_coverage_statistics(golden_vals: List[int], node_results: Dict):
    ...
    # 按 topk 分组（如果有多个 run，选择第一个或可以选择合并）
    files_by_topk = {}  # {topk: [filepath1, filepath2, ...]}
    files_by_topk_first = {}  # {topk: filepath} - 用于兼容原有逻辑
    for filepath in rank_files:
        ...
        if topk != float('inf'):
            ...
            # 如果是第一次遇到这个 topk，保存第一个文件路径
            if topk not in files_by_topk_first:
                files_by_topk_first[topk] = filepath
    ...
    # 2) 对每个 gnn_rank 文件统计覆盖率（包括所有 run 文件）
    print('\n[2/2] 统计各 gnn_rank 文件的覆盖率...')
    print('注意：将对每个 topk 的所有 run 文件进行统计')
    
    results = []  # 列表，每个元素为 {'topk': x, 'run': y, ...} 的字典
    
    total_runs = sum(len(files) for files in files_by_topk.values())
    current_run = 0
    ...
    for i, topk in enumerate(topk_list, 1):
        ...
        for rank_file in sorted(rank_files):  # 对每个 run 文件都统计
            current_run += 1
            filename = os.path.basename(rank_file)
            
            # 提取 run 编号
            run_match = re.search(r'_run(\d+)\.txt', filename)
            run_num = int(run_match.group(1)) if run_match else 1
            
            rank_tag = f'topk{topk}_run{run_num}'
            ...
            if not filtered_nodes:
                print(f'⚠️ [{current_run}/{total_runs}] {rank_tag}: 没有匹配的节点，跳过')
                continue
            ...
            if current_run % 50 == 0 or current_run == total_runs:
                print(f'  [{current_run}/{total_runs}] {rank_tag}: 节点={len(filtered_nodes)}, '
    ...
    with open(RESULT_CSV, 'w', newline='', encoding='utf-8') as f:
        ...
        # 写入列标题
        header = ['topk', 'run', 'tested_nodes', 'fault_coverage(%)', 'node_coverage(%)', 'avg_rmse', 'max_rmse', 'escaped_nodes', 'escaped_faults']
        writer.writerow(header)
        
        # 写入每个 run 的数据（每行代表一个 topk 的一个 run）
    ...
    for res in results:
        topk = res['topk']
        if topk not in runs_per_topk:
            runs_per_topk[topk] = 0
        runs_per_topk[topk] += 1
    ...
    # 显示每个 topk 的 run 数量
    if len(runs_per_topk) > 0:
        print(f'\n📊 每个 TopK 的运行次数:')
        for topk in sorted(runs_per_topk.keys()):
            print(f'   TopK={topk}: {runs_per_topk[topk]} runs')
